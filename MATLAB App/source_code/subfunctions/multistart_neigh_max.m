...
[max_weight_start max_weight_start_idx]=max(weight_edges_start);
[max_weight_end max_weight_end_idx]=max(weight_edges_end);

if isempty(max_weight_end)
    nextNode=neigh_start(max_weight_start_idx);
    insert_flag=1;
    previous=current_node_check(1);
    current_node_check=nextNode;
elseif max_weight_start>=max_weight_end
    nextNode=neigh_start(max_weight_start_idx);
    insert_flag=1;
    previous=current_node_check(1);
    current_node_check=nextNode;
elseif isempty(max_weight_start)
    nextNode=neigh_end(max_weight_end_idx);
    insert_flag=0;
    previous=current_node_check(2);
    current_node_check=nextNode;
elseif max_weight_end>max_weight_start
    nextNode=neigh_end(max_weight_end_idx);
    insert_flag=0;
    previous=current_node_check(2);
    current_node_check=nextNode;
